...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    setStatus(`Could not load your profile: ${err.message}`);
  }
});
...
function renderProfile() {
  profileCard.innerHTML = `
    ...
    <div>
      ...
      <button id="logoutBtn" class="ghost small-btn"><svg class="icon"><use href="#icon-logout"></use></svg>Logout</button>
    </div>`;
  ...
  projects.forEach((project) => {
    ...
    row.innerHTML = `
      ...
      <div class="project-actions">
        ${canAccept ? `<button data-accept="${project.id}"><svg class="icon"><use href="#icon-check"></use></svg>Accept Project</button>` : ""}
        ${canOpenChat(project) ? `<button data-chat="${project.id}"><svg class="icon"><use href="#icon-message"></use></svg>Open Chat</button>` : ""}
...
async function acceptProject(projectId) {
  ...
  try {
    ...
    await ensureChatDocument({ ...project, id: projectId, status: "accepted", acceptedDeveloperId: currentUser.uid, chatId });
    setStatus("Project accepted. Chat is ready.");
    activeMode = "mine";
    subscribeProjects();
  } catch (err) {
    setStatus(`Could not accept project: ${err.message}`);
  }
}
...
async function updateProjectStatus(projectId, status) {
  ...
  try {
    ...
    setStatus(`Project marked ${status}.`);
  } catch (err) {
    setStatus(`Could not update status: ${err.message}`);
    subscribeProjects();
  }
}
...
projectRequestForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    activeMode = "mine";
    setStatus("Website request submitted.");
    subscribeProjects();
  } catch (err) {
    setStatus(`Could not submit website request: ${err.message}`);
  }
});
...
async function openProjectChat(projectId) {
  ...
  try {
    await ensureChatDocument(project);
  } catch (err) {
    setStatus(`Could not prepare chat: ${err.message}`);
    return;
  }
...
async function ensureChatDocument(project) {
  const chatId = project.chatId || `project_${project.id}`;
  if (!project.customerId || !project.acceptedDeveloperId) throw new Error("This project has no active developer yet.");
  ...
  await setDoc(chatRef, {
    ...
    developerId: project.acceptedDeveloperId,
    participants: [project.customerId, project.acceptedDeveloperId],
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp()
  });
}
...
function subscribeToChat(chatId) {
  ...
  chatUnsub = onSnapshot(q, (snap) => {
    ...
    chatMessages.innerHTML = `<p class="note">Could not load chat: ${escapeHtml(err.message)}</p>`;
  });
}
...
function renderAttachment(file) {
  ...
  return `<a class="file-link" href="${url}" download="${name}"><svg class="icon"><use href="#icon-download"></use></svg>Download ${name}</a>`;
}
...
function renderThreads(projects) {
  ...
  projects.forEach((project) => {
    ...
    button.innerHTML = `<strong><svg class="icon"><use href="#icon-message"></use></svg>${escapeHtml(project.title || "Project")}</strong><span>${escapeHtml(project.status || "accepted")}</span>`;
...
async function loadAdminPanel() {
  ...
  let projectsSnap;
  try {
    [usersSnap, projectsSnap] = await Promise.all([getDocs(collection(db, "users")), getDocs(query(collection(db, "projects"), orderBy("createdAt", "desc")))]);
  } catch (err) {
    setStatus(`Could not load admin panel: ${err.message}`);
    return;
  }
