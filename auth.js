...
document.querySelector("#registerGoogle")?.addEventListener("click", async () => {
  if (!validateGoogleRegistrationFields()) return;
  try {
    authActionInProgress = true;
...
function validateGoogleRegistrationFields() {
  const requiredNames = ["name", "email", "phone", "address", "role"];
  const missingField = requiredNames
    .map((name) => registerForm?.elements[name])
    .find((field) => !String(field?.value || "").trim() || !field.checkValidity());

  if (!missingField) return true;

  authStatus.textContent = "Fill full name, email, phone, location, and role before using Google registration.";
  missingField.focus();
